...
void set_mixture_TRY(std::unique_ptr<IdealGasMix> & gas, const double T, 
    const double rho, const double *Y) {
    gas->setState_TRY(T, rho, Y);
}


double get_P(std::unique_ptr<IdealGasMix> & gas, const double rho, const double e, double *Y){
    double T = get_T(gas, rho, e, Y);
    return rho * GAS_CONSTANT / gas->meanMolecularWeight() * T;
} // end get_P


double get_T(std::unique_ptr<IdealGasMix> & gas, const double rho, const double e, double *Y){
    const double T_guess = 300.0;
    set_mixture_TRY(gas, T_guess, rho, Y);
    return get_temperature_from_energy(gas, e);
} // end get_T

double get_gamma(std::unique_ptr<IdealGasMix> & gas){
    return gas->cp_mass() / gas->cv_mass();
} // end get_gamma


double get_energy_from_temperature(std::unique_ptr<IdealGasMix> & gas, const double T_in){
    gas->setTemperature(T_in);
    return gas->intEnergy_mass();
} // end get_energy_from_temperature


double get_temperature_from_energy(std::unique_ptr<IdealGasMix> & gas, 
    const double e_in) {
